 ...
 *
 * 形态〇(自行注册进 DSH 0.1.5 原生右侧栏 sidebarRightTabs)已移除:
 * better-sidebar 的 native surface 会把本插件的页签本身镜像进原生右侧栏
 * (registerNativeSurface → sync() → tabs.register),自注册就得到两个同名
 * "GitHub Workbench" 页签;镜像那份还额外带 urlTarget/createTab 深链与设置
 * 同步,故保留镜像、删掉自注册(见 fork 提交说明)。
 */
