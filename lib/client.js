...
function inboxItemKey(owner, repo, n) {
  return `${owner}/${repo}#${n}`;
}
...
async function searchIssuesCreatedSince(repos, createdSinceIso, viewer) {
...
function mergeIncoming(prev, incoming, readKeys, selfKeys) {
  ...
  const items = [...map.values()].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, MAX_ITEMS);
  return { items, fresh };
}
...
function parseItems(raw) {
  ...
  try {
    ...
    return arr.filter((x) => x && typeof x === "object" && typeof x.key === "string" && typeof x.number === "number");
  } catch {
    return [];
  }
}
function parseRead(raw) {
  ...
  try {
    const arr = JSON.parse(raw);
    return Array.isArray(arr) ? new Set(arr.filter((x) => typeof x === "string").slice(0, MAX_READ)) : /* @__PURE__ */ new Set();
  } catch {
...
function createInboxStore(deps) {
  ...
  async function tick() {
    ...
    const incoming = hits.filter((h) => allowed.has(`${h.owner}/${h.repo}`)).map((h) => hitToItem(h, true));
  ...
  function markAllRead() {
    readKeys = /* @__PURE__ */ new Set([...readKeys, ...items.map((it) => it.key)]);
...
function InboxOverlay(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime7.jsxs)("div", { className: "gw-inbox", "data-gw-inbox": "", children: [
    ...
    snap.truncatedWatch && /* @__PURE__ */ (0, import_jsx_runtime7.jsx)("div", { className: "gw-pop-hint", style: { padding: "6px 12px" }, children: "\u4EC5\u76D1\u89C6\u6700\u8FD1\u63A8\u9001\u7684\u6700\u591A 300 \u4E2A\u516C\u5F00\u4ED3\u3002" }),
    ...
    /* @__PURE__ */ (0, import_jsx_runtime7.jsxs)("div", { className: "gw-list", children: [
      ...
      snap.hasToken && snap.items.length === 0 && !snap.lastError && /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(Empty, { children: "\u6700\u8FD1\u4E03\u5929\u6CA1\u6709\u516C\u5F00\u4ED3\u7684\u65B0 Issue\u3002" }),
      snap.hasToken && snap.items.map((it) => /* @__PURE__ */ (0, import_jsx_runtime7.jsxs)(
        "button",
        {
          type: "button",
...
function WorkbenchApp({ sessionId, visible, seedUrl }) {
  ...
  (0, import_react8.useEffect)(() => {
    inboxStore.setOnFresh(visible ? (fresh) => {
      const top = fresh[0];
      if (top) ui.toast(`\u65B0 Issue \xB7 ${top.owner}/${top.repo} #${top.number}`, "ok");
    } : null);
