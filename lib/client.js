...
function formatGhErrorDetail(body) {
...
function isRestIssuesListUrl(url) {
  try {
...
function isRestPullsListUrl(url) {
  try {
    const p = new URL(url, "https://api.github.com").pathname.replace(/\/$/, "");
...
function excludePullsFromIssueList(items) {
  return items.filter((it) => it.pull_request == null);
}
function filterPullsByMerged(items, wantMerged) {
...
async function ghRequest(path, opts = {}) {
  ...
  let upstream = "";
  try {
    upstream = formatGhErrorDetail(await res.json());
  } catch {
  }
...
function restSort(sort) {
  return { sort: sort === "updated" ? "updated" : "created", direction: "desc" };
}
async function listIssuesRest(ref, state, sort, pageUrl) {
  const path = pageUrl && isRestIssuesListUrl(pageUrl) ? pageUrl : `/repos/${ghRefKey(ref)}/issues${qs({ state, per_page: PAGE, ...restSort(sort) })}`;
  const { data, nextUrl } = await ghList(path);
  const items = excludePullsFromIssueList(data);
  return { items, nextUrl, totalCount: null };
}
async function listPullsRest(ref, filter, sort, pageUrl) {
  const state = filter === "open" ? "open" : "closed";
  const path = pageUrl && isRestPullsListUrl(pageUrl) ? pageUrl : `/repos/${ghRefKey(ref)}/pulls${qs({ state, per_page: PAGE, ...restSort(sort) })}`;
  const { data, nextUrl } = await ghList(path);
  const items = filter === "open" ? data : filterPullsByMerged(data, filter === "merged");
  return { items, nextUrl, totalCount: null };
}
function isSearchValidation(e) {
